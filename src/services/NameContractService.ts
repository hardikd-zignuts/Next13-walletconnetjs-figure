...
export class NameContractService {
  ...
  async getContractAddress(): Promise<string> {
    ...
    this.contractAddress = await this.wasmService.lookupContractByName(
      this.rootName
    );
    return this.contractAddress;
  }
  ...
  async getContractConfig(): Promise<QuerySettingsResponse> {
    return this.wasmService.queryWasmCustom<
      QuerySettings,
      QuerySettingsResponse
    >(await this.getContractAddress(), new QuerySettings());
  }

  async listNames(address: string): Promise<string[]> {
    const queryRes = await this.wasmService.queryWasmCustom<
      QueryNamesByAddress,
      QueryNamesByAddressResponse
    >(await this.getContractAddress(), new QueryNamesByAddress(address));
  ...
  async resolveName(name: string): Promise<string> {
    const queryRes = await this.wasmService.queryWasmCustom<
      QueryAddressByName,
      QueryAddressByNameResponse
    >(await this.getContractAddress(), new QueryAddressByName(name));
  ...
  async searchNames(search: string): Promise<NameMetaData[]> {
    const queryRes = await this.wasmService.queryWasmCustom<
      SearchNamesRequest,
      SearchNamesQueryResponse
    >(await this.getContractAddress(), new SearchNamesRequest(search));

    return queryRes.names;
  }

  async generateNameRegisterBase64Message(
    name: string,
    address: string
  ): Promise<string> {
    ...
    const message = new MsgExecuteContract()
      .setMsg(
        Buffer.from(
          new RegisterName().setName(name).toJson(),
          "utf-8"
        ).toString("base64")
      )
      .setFundsList([
        new Coin().setAmount(contractConfig.fee_amount).setDenom(FEE_DENOM),
      ])
