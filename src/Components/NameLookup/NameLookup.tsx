  ...
  return (
    <NameLookupWrapper>
      <Dropdown
        ...
        name="lookupType"
        value={lookupType}
        options={["select a lookup type", "name", "address"]}
        onChange={(t) => setLookupType(t)}
      ></Dropdown>
      <form>
        <Input
          label={lookupType.slice(0, 1).toUpperCase() + lookupType.slice(1)}
          value={value}
          onChange={(v) => setValue(v)}
        />
        <Button
          type="submit"
          disabled={!valueValid(value) || submitting}
      ...
      {!searchError && results.length > 0 && (
        <Results>
          ...
          <NameList>
            {results.map((res, i) => (
              <Name key={`res-${i}`}>{res}</Name>
            ))}
          </NameList>
        </Results>
      )}
      {!searchError && additionalNames.length > 0 && (
        <Results>
          <SubHeader>Additional Names:</SubHeader>
          <NameList>
            {additionalNames.map((res, i) => (
              <Name key={`additional-${i}`}>{res}</Name>
            ))}
          </NameList>
        </Results>
      )}
